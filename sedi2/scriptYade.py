...
class simulation():
  ...
  def __init__(self):
    ...
    O.periodic = True;
    O.cell.setBox(0.12,0.24,0.12);


    young = 5e6; density = 25000;
    ...
    fl = open('coords_r=0pt005.txt', 'r')
    for line in fl:
            x = float(line.split()[0]); y = float(line.split()[1]); z = float(line.split()[2]);
            O.bodies.append(sphere([x,y,z],0.00025, material='spheremat'))
    fl.close();
    ...
    newton=NewtonIntegrator(damping=0.00, gravity = (0.0 ,0.0, 0.0)) # set gravity in the openfoam side :file: constant/g
    #O.dt = 1e-07
	...
	GlobalStiffnessTimeStepper(timestepSafetyCoefficient=0.1, label = "ts"),
        ...
        PyRunner(command='sim.printMessage()', iterPeriod= 100, label='message'),
        PyRunner(command='sim.saveSim()', iterPeriod= 15000, label='message'),
	newton,
